...
internal class Program
{
	...
	{
		// == Create the client
		DiscordRpcClient client = new("424087019149328395", discordPipe)
		{
	...
	private static void FullClientExample()
	{
		...
		using (client = new DiscordRpcClient("424087019149328395", //The client ID of your Discord Application
			       discordPipe, //The pipe number we can locate discord on. If -1, then we will scan.
	...
	private static async void ReadyTaskExample()
	{
		...
		using DiscordRpcClient client = new("424087019149328395", discordPipe)
		{
			Logger = new ConsoleLogger(logLevel, true)
		};

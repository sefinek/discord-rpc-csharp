...
public sealed class DiscordRpcClient : IDisposable
{
	...
	{
		return Update(p => p.Buttons = buttons);
	}
	...
	{
		return Update(p => p.Buttons[index] = button);
	}
	...
	{
		return Update(p => p.Details = details);
	}
	...
	{
		return Update(p => p.Secrets = secrets);
	}
	...
	{
		return Update(p => p.Timestamps = null);
	}
	...
	{
		UriSchemeRegister urischeme = new(_logger, ApplicationID, steamAppID, executable);
